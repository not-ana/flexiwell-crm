// ClassPass Webhook Signature Verification
...
/**
 * Verify ClassPass webhook signature using HMAC-SHA256
 * The signature is sent in the X-ClassPass-Signature header
 *
 * @param requestBody - Raw request body (string or Buffer)
 * @param signature - Value from X-ClassPass-Signature header
 * @returns true if signature is valid, false otherwise
 */
export async function verifyClassPassSignature(
  requestBody: string | Buffer,
  signature: string
): Promise<boolean> {
  try {
    const credentials = await getClassPassCredentials();
    if (!credentials?.webhookSecret) {
      console.error("ClassPass webhook secret not configured");
      return false;
    }
    ...
    console.error("Error verifying ClassPass signature:", error);
    return false;
  }
}
...
export async function verifyAndParseWebhook<T>(
  ...
  if (!signatureHeader) {
    return {
      valid: false,
      data: null,
      error: "Missing X-ClassPass-Signature header",
    };
  }

  const isValid = await verifyClassPassSignature(rawBody, signatureHeader);
  if (!isValid) {
    return {
      valid: false,
      data: null,
